...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/admin/users` | 전체 사용자 목록 (기록 수 포함) |
| GET | `/admin/logs` | 활동 로그 조회 |
| GET | `/admin/stats` | 전체 사용자·기록·로그 수 |

> 인증이 필요한 창구는 `Authorization: Bearer <토큰>` 헤더가 있어야 합니다. `/docs` 의 **Authorize** 버튼으로도 테스트할 수 있습니다. 관리자 창구는 `role=admin` 사용자만 접근 가능(그 외 403).

### 주요 기능

- **로그인/회원가입**: OAuth2 password flow + JWT 토큰, bcrypt 비밀번호 해시.
- **사용자별 기록 분리**: 로그인한 사용자의 기록만 조회·수정·삭제 가능.
- **관리자 대시보드**: 첫 가입자는 관리자. 전체 사용자·활동 로그 조회. `/ui` 에 관리자 패널 표시.
- **활동 로그**: 회원가입·로그인·기록 변경을 자동 기록.
